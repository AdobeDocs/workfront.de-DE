...
title: HTTP &gt; Erstellen eines einfachen Autorisierungsanfragemoduls
...
# [!UICONTROL HTTP] >[!UICONTROL Erstellen eines einfachen Autorisierungsanfragemoduls]

Mit diesem [!DNL Adobe Workfront Fusion] -Modul können Sie eine HTTP-Anforderung mit HTTP-grundlegender Autorisierung konfigurieren und an einen Server senden. Die empfangene HTTP-Antwort ist dann im Ausgabebundle enthalten.
...
## [!UICONTROL HTTP] >[!UICONTROL Erstellen einer grundlegenden Autorisierungsanforderung] Modulkonfiguration

Wenn Sie das Modul [!UICONTROL HTTP] >[!UICONTROL Erstellen einer einfachen Autorisierungsanfrage] konfigurieren, zeigt [!DNL Adobe Workfront Fusion] die unten aufgeführten Felder an. Ein fett hervorgehobener Titel in einem Modul zeigt ein erforderliches Feld an.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>Wählen Sie den Schlüssel aus, der Ihre grundlegenden Authentifizierungsberechtigungen enthält, oder klicken Sie auf <strong>[!UICONTROL Add]</strong> , um Ihre Anmeldeinformationen einem neuen Schlüssel hinzuzufügen. </p> <p>Hinweis: Sie können weitere Anmeldedaten hinzufügen, um mühelos zwischen den einzelnen Verbindungen zu wechseln.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Der HTTP-Hauptteil sind die Daten-Bytes, die in einer HTTP-Transaktionsnachricht unmittelbar auf die Header übertragen werden, sofern welche verwendet werden sollen.</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>Dieser Nachrichtentyp wird zu [!UICONTROL POST]-Daten mit <code>[!UICONTROL application/x-www-form-urlencoded]</code> verwendet.</p> <p>Bei <code>[!UICONTROL application/x-www-form-urlencoded]</code> ist der Hauptteil der an den Server gesendeten HTTP-Nachricht im Wesentlichen eine Abfragezeichenfolge. Die Schlüssel und Werte werden in Schlüssel-Wert-Paaren kodiert, die durch <code>&amp;</code> getrennt sind, und in einem <code>=</code> zwischen Schlüssel und Wert. </p> <p>Verwenden Sie stattdessen für Binärdaten "<code>multipart/form-data</code>".</p> 
