...
title: HTTP &gt; Erstellen einer API-Schlüsselautorisierungsanforderung
description: Dieses [!DNL Adobe Workfront Fusion] Aktionsmodul sendet eine HTTPS-Anforderung an eine angegebene URL, für die eine Autorisierung des API-Schlüssels erforderlich ist, und verarbeitet die Antwort.
author: Becky
...
# HTTP >[!UICONTROL Erstellen einer API-Schlüsselautorisierungsanforderung]

>[!NOTE]
>
>[!DNL Adobe Workfront Fusion] erfordert zusätzlich zu einer Adobe Workfront-Lizenz eine [!DNL Adobe Workfront Fusion] -Lizenz.

Dieses Aktionsmodul [!DNL Adobe Workfront Fusion] sendet eine HTTPS-Anfrage an eine angegebene URL, für die eine Autorisierung des API-Schlüssels erforderlich ist, und verarbeitet die Antwort.
...
Weitere Informationen zu [!UICONTROL Adobe Workfront Fusion]-Lizenzen finden Sie unter [Adobe Workfront Fusion-Lizenzen](../../../workfront-fusion/get-started/license-automation-vs-integration.md)

## [!UICONTROL HTTP] >[!UICONTROL Erstellen einer API-Schlüsselautorisierungsanforderung] Modulkonfiguration

Wenn Sie das Modul [!UICONTROL HTTP] >[!UICONTROL Authorization request] für API-Schlüssel erstellen, zeigt [!DNL Adobe Workfront Fusion] die unten aufgeführten Felder an. Ein fett hervorgehobener Titel in einem Modul zeigt ein erforderliches Feld an.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>Wählen Sie den Schlüssel aus, der Ihre Anmeldedaten für die API-Schlüsselauthentifizierung enthält. Um einen neuen Schlüssel hinzuzufügen, klicken Sie auf <strong>[!UICONTROL Add]</strong> und konfigurieren Sie die folgenden Informationen:</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL API Key Parameter name]</strong> </p> <p>Geben Sie den Namen ein, mit dem der API-Aufruf den API-Schlüssel identifiziert, z. B. "apiKey"oder "X-API-Schlüssel". Diese Informationen finden Sie in der Dokumentation des Webdienstes, mit dem das Modul eine Verbindung herstellt.</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Der HTTP-Hauptteil sind die Daten-Bytes, die in einer HTTP-Transaktionsnachricht unmittelbar auf die Header übertragen werden, sofern welche verwendet werden sollen.</p> 
    <ul> 
     <li> <p><strong>[!UICONTROL Raw]</strong> </p> <p>Der Rohkörpertyp eignet sich im Allgemeinen für die meisten HTTP-Textkörperanforderungen, selbst wenn in der Entwicklerdokumentation keine zu sendenden Daten angegeben sind.</p> <p>Geben Sie im Feld [!UICONTROL Inhaltstyp] eine Form zum Analysieren der Daten an.</p> <p>Ungeachtet des ausgewählten Inhaltstyps gibt das Modul Daten in einem beliebigen Format ein, das in der Entwicklerdokumentation festgelegt oder benötigt wird.</p> </li> 
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>Dieser Nachrichtentyp wird zu [!UICONTROL POST]-Daten mit <code>application/x-www-form-urlencoded</code> verwendet.</p> <p>Bei <code>[!UICONTROL application/x-www-form-urlencoded]</code> ist der Hauptteil der an den Server gesendeten HTTP-Nachricht im Wesentlichen eine Abfragezeichenfolge. Die Schlüssel und Werte werden in Schlüssel-Wert-Paaren kodiert, die durch <code>&amp;</code> getrennt sind, und in einem <code>=</code> zwischen Schlüssel und Wert. </p> <p>Verwenden Sie stattdessen für Binärdaten "<code>[!UICONTROL multipart/form-data]</code>".</p> 
