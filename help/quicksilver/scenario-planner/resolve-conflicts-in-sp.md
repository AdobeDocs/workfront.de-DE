...
# Lösung von Initiativkonflikten im [!DNL Scenario Planner]
...
## Überblick über die Behebung von Konflikten

* Ein Konflikt wird auch als eine Überzuweisung der Arbeitsplatzrollen oder des Budgets eines Szenarios verstanden.
* Wann [!DNL Workfront] einen Konflikt feststellt, wird der Balken, der dem widersprüchlichen Monat während der Dauer der Initiative entspricht, rot angezeigt. Dies kann in einem der folgenden Fälle passieren:
...
>[!TIP]
>
>Standardmäßig wird die [!DNL Scenario Planner] geht davon aus, dass Sie für ein Szenario eine Budgetvorgabe von 0 Stellenrollen und für ein Szenario eine von 0 USD bzw. eine Gegenleistung von 0 USD in der Währung Ihres Systems erhalten haben, sofern Sie nichts anderes angegeben haben. Die Anzahl der Arbeitsplätze gibt die Anzahl der FTEs (Vollzeitäquivalente) oder Stunden an, die für die Stellenfunktion vorgesehen sind.
>
>Für alle Berechnungen im Szenario-Planer verwendet Workfront den folgenden Wert: 1 VZÄ = 8 Stunden.
>
...
* Sie können einen Konflikt lösen, indem Sie einen der folgenden Schritte ausführen:
   ...
   * Anpassen der Auftrags- und Budgetressourcen für das Szenario durch Bearbeiten des Plans. Weitere Informationen finden Sie unter [Erstellen und Bearbeiten von Plänen im [!DNL Scenario Planner]](../scenario-planner/create-and-edit-plans.md).
...
1. (Optional) Wählen Sie im **[!DNL Initial scenario]** aus dem Dropdown-Menü das Szenario auswählen, das Sie überprüfen möchten.

   >[!TIP]
   >
   >Ein Plan kann mehrere Szenarien haben. Bei der Prüfung der Konflikte des Plans [!DNL Workfront] bezieht sich auf die Ressourcen, die derzeit für das ausgewählte Szenario verfügbar sind, sowie auf die Ressourcen, die für die Initiativen dieses Szenarios erforderlich sind. Weitere Informationen zu Szenarien finden Sie unter [Erstellen und Vergleichen von Planszenarien in [!DNL Scenario Planner]](../scenario-planner/create-and-compare-scenarios-for-a-plan.md).
...
1. Führen Sie einen der folgenden Schritte aus, um mehr über mögliche Konflikte zu erfahren:
   ...
   * Wenn Sie sich den Plan nach Monat ansehen, bewegen Sie den Mauszeiger über einen Monat in der Zeitleiste des Plans, um die erforderlichen Ressourcen für diesen Monat anzuzeigen und zu sehen, ob die Konflikte für den Monat Personen oder kostenbezogen sind.
      ...
      * Verfügbare, erforderliche und überverteilte Kosten für den Monat für alle für diesen Monat geplanten Initiativen
         ...
         >[!TIP]
         >
         >Die [!UICONTROL Verfügbar] Kosten sind das Budget des Szenarios für diesen Monat.
       ...
       <tr> 
        ...
        <td> <p>Die mit dieser Initiative verbundenen Arbeitsplatzrollen, die für den ausgewählten Monat überverteilt werden. In den folgenden Spalten werden Informationen zu den einzelnen für den ausgewählten Monat erforderlichen Auftragsrollen angezeigt, die in Konflikt mit der Anzahl der für diesen Monat verfügbaren Jobrollen stehen:</p> 
         <ul> 
          ...
          <li> <p><strong>[!UICONTROL Erforderlich]</strong>: Die für die Initiative für den ausgewählten Monat erforderliche Anzahl von Aufgaben.</p> </li> 
          <li> <p style="font-weight: normal;"><strong>[!UICONTROL Überzugewiesen]:</strong> Differenz zwischen der für die Initiative erforderlichen Zahl und der für das Szenario verfügbaren Zahl. </p> </li> 
         </ul> <p>Tipp: Manchmal stimmt die Anzahl der [!UICONTROL Verfügbaren] Rollen mit der Anzahl der [!UICONTROL Erforderlichen] Rollen überein oder ist höher, aber die [!DNL Scenario Planner] zeigt weiterhin eine Überzuweisung an. Dies bedeutet, dass es Initiativen mit höherem Rang gibt, die bereits die im Plan für den gleichen Monat verfügbaren Arbeitsplatzrollen genutzt haben. </p> </td> 
       </tr> 
       <tr> 
        ...
        <td> <p>Die Kosten der Initiative für den ausgewählten Monat. Die folgenden Spalten enthalten Informationen zu den benötigten Kosten und dem für den ausgewählten Monat verfügbaren Budget:</p> 
         <ul> 
          ...
          <li> <p style="font-weight: normal;"><strong>[!UICONTROL Überzugewiesen]:</strong> Differenz zwischen den Kosten der Initiative und dem im Rahmen des Plans verfügbaren Budget. </p> </li> 
         </ul> <p>Tipp: Manchmal entsprechen die [!UICONTROL Verfügbaren] Kosten den [!UICONTROL Erforderlichen] Kosten der Initiative für den ausgewählten Monat und die [!DNL Scenario Planner] weist noch immer eine Überzuweisung der Kosten auf. Das bedeutet, dass es Initiativen mit höherem Rang gibt, die bereits das für den gleichen Monat verfügbare Budget des Plans verwenden. </p> </td> 
       </tr> 
      </tbody> 
     </table>
...
1. Führen Sie einen der folgenden Schritte aus, um das Bedienfeld mit den Details zur Initiative zu öffnen und weitere Informationen darüber anzuzeigen, wo die Konflikte auftreten, und um sie zu beheben:
   ...
   * Klicken Sie auf **[!UICONTROL Mehr]** icon ![](assets/more-icon.png) rechts neben dem Namen der Initiative klicken Sie auf **[!UICONTROL Bearbeiten]**.
...
1. (Bedingt) Für Initiativen mit Konflikten in der Rolle &quot;Job&quot;gehen Sie zu **[!UICONTROL Erforderliche Vorgangsrollen]** -Abschnitt, um alle für Ihre Initiative erforderlichen Arbeitsplatzrollen anzuzeigen. Ermitteln Sie, welche Auftragsrollen möglicherweise übergeordnet sind. Überprüfen Sie die Anzahl der erforderlichen FTEs oder Stunden für jede Stellenaufgabe für jeden Monat der Initiative. Das Feld mit der FTE-Nummer oder der Stundenzahl für Monate mit Überallokationen wird in einer roten Umrisslinie angezeigt.
...
1. (Optional) Klicken Sie auf **[!UICONTROL Details anzeigen]** unter der Stellenrolle, die einen Konflikt anzeigt, um zu sehen, wo die Konflikte auftreten, und um die Konfliktmonate im Diagrammbereich des Plans hervorzuheben. Für jede Auftragsrolle werden zusätzliche Informationen angezeigt.
   ...
   >[!TIP]
   >
   >In der Übersicht des Plans werden in den Monaten, in denen die Aufgaben zugewiesen werden, der Name und die Anzahl der Rollen angezeigt, die für jede Initiative benötigt werden, wo sie benötigt wird. Sie müssen die [!UICONTROL Monat] Ansicht zum Anzeigen des Namens der Auftragsrollen
...
1. Führen Sie einen der folgenden Schritte aus, um Konflikte mit Auftragsrollen zu lösen:
   ...
   * Bewegen Sie den Mauszeiger über den Namen der Auftragsrolle und klicken Sie auf **[!UICONTROL delete] icon** ![](assets/delete.png) die Rolle der Beschäftigung aus der Initiative zu streichen.
   * Auswählen **[!UICONTROL Rollen zu den verfügbaren Ressourcen des Szenarios hinzufügen]** Klicken Sie auf **[!UICONTROL Anwenden]**.

      Dadurch wird die fehlende Anzahl von FTEs für die Rolle als Arbeitskraft oder Stunden zum Szenario [!UICONTROL Verfügbar] -Feld.

      >[!NOTE]
      >
      >Die Rollen, die Sie zum Beheben von Konflikten hinzufügen, ändern die [!UICONTROL Verfügbar] Vorgangsrollen für das ausgewählte Szenario und nicht für alle Szenarien im Plan.

      Ein nach oben zeigender grüner Pfeil ![](assets/upward-green-arrow.png) zeigt den Monat in der Zeitleiste des Plans an, um anzugeben, dass dem Plan in diesem Monat weitere Ressourcen hinzugefügt wurden. Sie müssen die [!UICONTROL Monat] anzeigen, um diesen Indikator zu sehen.

   * (Bedingt) Schließen Sie das Detailbedienfeld und geben Sie der Initiative eine höhere Priorität, um nach Möglichkeit zunächst Haushaltsmittel aus dem Plan zu erhalten. Informationen zur Aktualisierung der Priorität von Initiativen finden Sie unter [Aktualisierung der Initiativprioritäten im Szenario-Planer](../scenario-planner/prioritize-initiatives.md).

1. (Optional) Klicken Sie auf **[!UICONTROL Details ausblenden]** , um das zusätzliche Detailfeld zu schließen, und klicken Sie auf **[!UICONTROL Anwenden]** , um die Änderungen zu speichern, die Sie an den Auftragsrollen vornehmen.

1. (Bedingt) Für Initiativen mit Kostenkonflikten gehen Sie zu **[!UICONTROL Kosten]** im Bereich &quot;Details der Initiative&quot;, um die Kosten für jeden Monat der Dauer der Initiative zu überprüfen. Finden Sie heraus, in welchen Monaten im Budget des Plans möglicherweise nicht genügend Geld zur Verfügung steht, um die Kosten für die ausgewählte Initiative zu decken. Das Feld mit dem nicht ausreichenden verfügbaren Budget wird in einem roten Umriss angezeigt.
...
1. (Optional) Klicken Sie auf **[!UICONTROL Details anzeigen]** unter den Kostenangaben, um zu sehen, wo der Konflikt auftritt, und um die Konfliktmonate in der Planung hervorzuheben. Für jeden Kostentyp werden die folgenden zusätzlichen Felder angezeigt:
   ...
   >[!NOTE]
   >
   >Wenn Sie die [!UICONTROL Personenkosten einschließen] Einstellung für die [!UICONTROL Budget] bei der Erstellung des Plans das Feld [!UICONTROL Personenkosten] -Zeile wird für keine Initiative in jedem Szenario angezeigt. In diesem Fall berücksichtigt Workfront keine Personenkosten in Berechnungen, um Kostenkonflikte zu ermitteln. Weitere Informationen zum Erstellen eines Plans finden Sie unter [Erstellen und Bearbeiten von Plänen im [!DNL Scenario Planner]](../scenario-planner/create-and-edit-plans.md).

1. Führen Sie einen der folgenden Schritte aus, um Kostenkonflikte zu lösen:

   * Anzahl der [!UICONTROL Feste Kosten] für jeden Monat der Initiative auf eine niedrigere Zahl.
   * Im **[!UICONTROL Erforderliche Vorgangsrollen]** -Abschnitt, passen Sie die Anzahl der Stellenrollen für den Monat nach Möglichkeit mit einem Budget für People-Kosten manuell an. Dadurch wird die Anzahl der Personenkosten reduziert.

      >[!TIP]
      >
      >Sie können die Kosten für Personen nicht manuell anpassen.

   * Auswählen **[!UICONTROL Fügen Sie dem Budget des Szenarios einen Betrag hinzu.]** Klicken Sie auf **[!UICONTROL Anwenden]**.
   ...
   * (Bedingt) Schließen Sie das Detailbedienfeld und geben Sie der Initiative eine höhere Priorität, um nach Möglichkeit zunächst Haushaltsmittel aus dem Plan zu erhalten. Informationen zur Aktualisierung der Priorität von Initiativen finden Sie unter [Aktualisierung der Prioritäten der Initiative in [!DNL Scenario Planner]](../scenario-planner/prioritize-initiatives.md).

1. Klicken **[!UICONTROL Anwenden]** wenn Sie Änderungen am Kostensegment vornehmen.
